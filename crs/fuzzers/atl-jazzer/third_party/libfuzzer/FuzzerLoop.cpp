...
// Coverage reproduction mode helper implementations
time_t Fuzzer::GetCoverageReproBaseTimestamp() {
  const char* base_timestamp_env = getenv("ATLJAZZER_COV_REPRO_BASE_TIMESTAMP");
  if (!base_timestamp_env || base_timestamp_env[0] == '\0') {
    return 0;
  }
  return std::strtoll(base_timestamp_env, nullptr, 10);
}

void Fuzzer::SortCorpusByMtime(std::vector<SizedFile> &CorporaFiles) {
  std::map<std::string, time_t> file_mtime_map;
  for (auto &SF : CorporaFiles) {
    struct stat st;
    file_mtime_map[SF.File] = (stat(SF.File.c_str(), &st) == 0) ? st.st_mtime : 0;
  }
  
  std::stable_sort(CorporaFiles.begin(), CorporaFiles.end(),
                  [&file_mtime_map](const SizedFile &A, const SizedFile &B) {
                    return file_mtime_map[A.File] < file_mtime_map[B.File];
                  });
}

void Fuzzer::PrintCoverageReproStats(const std::string &FilePath, time_t base_timestamp) {
  struct stat st;
  if (stat(FilePath.c_str(), &st) == 0) {
    time_t relative_time = st.st_mtime - base_timestamp;
    char label[64];
    snprintf(label, sizeof(label), "REPRO-%ld", relative_time);
    PrintStats(label);
  }
}
...
void Fuzzer::ReadAndExecuteSeedCorpora(std::vector<SizedFile> &CorporaFiles) {
  ...
  if (CorporaFiles.empty()) {
    ...
    time_t base_timestamp = GetCoverageReproBaseTimestamp();
    bool coverage_repro_mode = (base_timestamp > 0);

    if (coverage_repro_mode) {
      Printf("INFO: Coverage reproduction mode enabled with base timestamp: %ld\n", base_timestamp);
      SortCorpusByMtime(CorporaFiles);
      Printf("INFO: Sorted %zd corpus files by modification time\n", CorporaFiles.size());
    }

    // Load and execute inputs one by one.
    for (size_t i = 0; i < CorporaFiles.size(); i++) {
      auto &SF = CorporaFiles[i];
      ...
      if (coverage_repro_mode) {
        PrintCoverageReproStats(SF.File, base_timestamp);
      }
