...
# Patterns for coverage reproduction mode: #N REPRO-<elapsed_time> cov: X ...
OLD_LIBFUZZER_REPRO_LINE_PTRN = re.compile(r"^#(\d+)\s+REPRO-(\d+).*cov: (\d+)")
LIBFUZZER_REPRO_LINE_PTRN = re.compile(r"^(\d+)\s#(\d+)\s+REPRO-(\d+).*cov: (\d+)")
...
def _parse_repro_line(line: str) -> Optional[Tuple]:
    """Parse coverage reproduction mode lines.
    E.g., "#2	REPRO-6 cov: 17 ft: 17 corp: 1/1b exec/s: 0 rss: 1502Mb"
    Returns: (elapsed_time, cov) or None
    """
    # Try new format first (with timestamp)
    match = LIBFUZZER_REPRO_LINE_PTRN.match(line)
    if match:
        # timestamp = int(match.group(1))  # Not used
        # roundno = int(match.group(2))  # Not used
        elapsed_time = int(match.group(3))  # This is the relative time from base timestamp
        cov = int(match.group(4))
        return elapsed_time, cov
    
    # Try old format (without timestamp)
    match = OLD_LIBFUZZER_REPRO_LINE_PTRN.match(line)
    if match:
        # roundno = int(match.group(1))  # Not used
        elapsed_time = int(match.group(2))  # This is the relative time from base timestamp
...
def parse_log_in_stream(
    ...
    def sync_result(data: dict):
        ...
        # Always include repro_cov_over_time if it has data
        if repro_cov_over_time:
            data["repro_cov_over_time"] = repro_cov_over_time
    ...
    try:
        ...
        for line in file_obj:
            ...
            # 0. repro line: repro coverage over time
            rslt = _parse_repro_line(decoded_line)
            if rslt:
                elapsed_time, cov = rslt
                repro_cov_over_time.append((elapsed_time, cov))
